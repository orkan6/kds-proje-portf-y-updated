...
class MarketDataService {
     ...
     */
    async getSP500Data(startDate) {
        try {
            // Get latest available date
            const [dateRow] = await this.query('SELECT MAX(Date) as maxDate FROM sp500_index');
            const maxDate = new Date(dateRow[0].maxDate);

            // Adjust start date if it's in the future or not provided
            let effectiveStartDate = startDate;
            if (!startDate || startDate > maxDate) {
                effectiveStartDate = new Date(maxDate);
                effectiveStartDate.setFullYear(maxDate.getFullYear() - 3); // Default 3 years
            }

            const formattedDate = format(effectiveStartDate, 'yyyy-MM-dd');
            ...
            if (!data || data.length === 0) {
                // Return empty approach or try fetching last N records as fallback
                const [fallbackData] = await this.query(`
                    SELECT Date, \`S&P500\` as price
                    FROM sp500_index
                    WHERE \`S&P500\` IS NOT NULL
                    ORDER BY Date DESC
                    LIMIT 252
                `);
                ...
                return {
                    dates: fallbackData.map(row => row.Date),
                    prices: fallbackData.map(row => parseFloat(row.price))
                };
            }
     ...
     */
    async getAssetPrices(symbol, startDate) {
        try {
            // 1. Get Max Date from DB to handle stale data issues
            const [dateRow] = await this.query('SELECT MAX(Date) as maxDate FROM sp500veri');
            const maxDate = new Date(dateRow[0].maxDate);

            // 2. Adjust start date: if requested startDate is in future relative to data, shift window
            let effectiveStartDate = startDate;
            if (!startDate || new Date(startDate) > maxDate) {
                effectiveStartDate = new Date(maxDate);
                effectiveStartDate.setFullYear(maxDate.getFullYear() - 1); // Default 1 year lookback from max data
            }
            ...
            // ... Check if commodity ...
            const isCommodity = querySymbol.includes('BRENT') || this.isCommodity(querySymbol);
            const tableName = isCommodity ? 'commodity_futures' : 'sp500veri';
            ...
            if (!rows || rows.length === 0) {
                // Fallback: Get last 252 points regardless of date
                const [fallbackRows] = await this.query(`
                    SELECT Date, \`${querySymbol}\` as price
                    FROM ${tableName}
                    WHERE \`${querySymbol}\` IS NOT NULL
                    ORDER BY Date DESC
                    LIMIT 252
                `);
                ...
                fallbackRows.sort((a, b) => new Date(a.Date) - new Date(b.Date));
            ...
            throw new Error('Varlık fiyat verisi alınamadı'); // Generic error to avoid exposing DB/Symbol details too much
        }
    }
