...
class AnalysisService {
    ...
    async getPerformanceComparison(portfolioId) {
        try {
            ...
            // FORCE END DATE to June 2024 as requested
            // We can still query DB for exact last trading day, or just hardcode if that's what user implies.
            // Using DB max date is safer but we ensure it doesn't go beyond 2024.
            const [dateRow] = await this.pool.query('SELECT MAX(Date) as maxDate FROM sp500_index');
            let endDate = new Date(dateRow[0].maxDate);

            // Safety cap: If DB has 2026 data but we know portfolio data ends in 2024, cap it.
            // User says "Jun2024".
            ...
            // 1. Fetch Portfolio Asset Prices (filtered by date)
            ...
            // 2. Fetch Market Data (filtered by date)
            const marketData = await this.marketDataService.getSP500Data(startDate);

            // Align Dates: Use Market Data dates as the master axis
            const masterDates = marketData.dates.filter(d => {
                const dateObj = new Date(d);
                return dateObj >= startDate && dateObj <= endDate;
            });

            const marketPrices = marketData.prices.slice(
                marketData.dates.indexOf(masterDates[0]),
                marketData.dates.indexOf(masterDates[masterDates.length - 1]) + 1
            );

            // 3. Calculate Portfolio Value for each master date
            const portfolioValues = masterDates.map(dateStr => {
                const rowDate = new Date(dateStr).toISOString().split('T')[0];
                let totalValue = 0;

                portfolio.forEach(asset => {
                    const prices = historicalPrices[asset.asset_symbol];
                    if (!prices) return;

                    // Match date YYYY-MM-DD
                    let priceData = prices.find(p => new Date(p.date).toISOString().split('T')[0] === rowDate);

                    // Fallback: previous price
                    if (!priceData) {
                        // Assuming prices sorted ASC
                        for (let i = prices.length - 1; i >= 0; i--) {
                            if (new Date(prices[i].date).toISOString().split('T')[0] < rowDate) {
                                priceData = prices[i];
                                break;
                            }
                        }
            ...
            // Normalize (Base 100)
            // Find first non-zero valid index
            const firstValidIdx = portfolioValues.findIndex(v => v > 0);
            const initialVal = firstValidIdx >= 0 ? portfolioValues[firstValidIdx] : 0;

            const normalizedPortfolio = portfolioValues.map((v, i) => {
                if (i < firstValidIdx) return 100;
                return initialVal > 0 ? (v / initialVal) * 100 : 100;
            });

            // Normalize Market
            const initialMarket = marketPrices[0];
            const normalizedMarket = marketPrices.map(v => (v / initialMarket) * 100);
